...
namespace our
{
    ...
    {
        // TODO: (Req 7) Write this function
        // Call the setup function of the parent class (TintedMaterial)
        TintedMaterial::setup();
        shader->set("alphaThreshold", alphaThreshold);

        glActiveTexture(GL_TEXTURE0);
        // Check if the texture and sampler are valid
        if (texture && sampler)
        {
            texture->bind(); // Binding to texture unit
            sampler->bind(0);
            shader->set("tex", 0); // Send texture unit number to the "tex" uniform

            // shader->set("tex", texture->getOpenGLName()); // Send texture unit number to the "tex" uniform
        }
    }
